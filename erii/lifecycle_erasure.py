...
def _file_profiles(root: Path) -> Dict[str, Tuple[Path, Mapping[str, Any]]]:
    ...
    for path in root.glob("*/*/relationship.json"):
        ...
        if relationship_id in profiles:
            raise ErasureSelectionError("relationship identity is ambiguous in staging")
...
def _digest_path(directory: str, stable_id: str) -> Path:
    return Path(directory, f"{hashlib.sha256(stable_id.encode('utf-8')).hexdigest()}.json")
...
def _raw_record_events(record: Mapping[str, Any]) -> Tuple[str, ...]:
    ...
    if not isinstance(events, Sequence) or isinstance(events, (str, bytes)):
        raise ErasureSelectionError("relationship adjudication events are malformed")
    return tuple(
...
def _cascade_event_deletions(
    ...
    *,
    ...
    """Closes event deletion over journal and causal dependencies."""
    ...
    try:
        ...
        raise ErasureSelectionError("relationship event history is malformed") from exc
    ...
    while changed:
        ...
        for raw_event in all_raw_events:
            ...
            if references.intersection(removed_events):
                ...
                changed = True
    # A later causal addition can make its owning record newly removable.
    for record in adjudications:
    ...
    *,
    ...
    for row in rows:
        try:
            ...
            raise ErasureSelectionError(f"{table} contains malformed JSON") from exc
        if not isinstance(raw, Mapping):
...
def _estimate_file_scope(
    ...
    if selector.scope is ErasureScope.COMPLETE_USER:
        ...
        user_id = _required_text(selector.user_id, "user_id")
        identity_id = _required_text(selector.user_identity_id, "user_identity_id")
...
def _open_sqlite_read_only(path: Path) -> sqlite3.Connection:
    try:
        ...
        raise ErasureSelectionError("SQLite staging copy is unreadable") from exc
...
def inspect_erasure_scope(
    ...
    """Validates an exact scope using read-only raw storage access."""
