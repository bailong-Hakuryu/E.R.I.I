...
可携带性，而不是无边界扩大框架适配器数量。`0.4.0b1` 已作为源码基线接受，rc1 已
完成采用、兼容与源码收口，当前 `0.4.0` 是稳定源码里程碑；`0.x` 是源码演进里程碑，
不要求逐个发布包。新角色领域模型应进入 v0.5 设计，而不是伪装成 `0.4.x` 修复。

当前贡献分两条轨道：

- **内核演进轨**当前维护 `0.4.x` 的缺陷、安全、兼容、Golden Path 与文档，并设计
  v0.5 的下一条完整纵切；正式包发布流程留到 `1.0`；
