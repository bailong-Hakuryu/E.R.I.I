...
> Product status: Experimental, removable, offline C0 prototype
> Persistence: none
> Network access: none

This is the only current C0 implementation status page. The canonical product
...
- No integration with `ERIIEngine`/TurnLedger ownership, Continuity delivery, REST,
  TypeScript SDK, MemoryPack, Backup or erase/rebuild.
- No persisted Interior Scene, Pending Residue, user-visible Thought Projection
  or Private Reflection.
- No product Staged/Adaptive orchestration, direct fallback, reviewer ensemble,
  Session Residue or multi-provider failover. D2/D3 currently exist only as
  deterministic offline Shadow fixtures.
...
| --- | --- | --- | --- |
| C0 contracts | `python -m pytest -q tests/deliberation -rxX` | `197 passed in 2.93s` | `0` |
...
| Repository regression | `python -m pytest -q tests --ignore=tests/test_full_erii_real_api_integration.py --ignore=tests/test_real_api_integration_simple.py -rxX --tb=short` | `942 passed, 5 skipped, 96 warnings, 488 subtests passed in 119.42s` | `0` |
| Optimized authority/binding regression | `python -O -m pytest -q tests/deliberation/test_authority_envelope_validation.py tests/deliberation/test_result_binding_regression.py tests/labs/deliberation/shadow_eval/test_integration.py` | `30 passed, 1 warning in 0.61s` | `0` |
