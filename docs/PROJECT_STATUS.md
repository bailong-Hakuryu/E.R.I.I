...
>
> Refactoring program: `R2` / `in_progress` (`2026-08-13` to `2026-09-27`)
...
Current gate: Complete the R2 Lifecycle Contracts, Inspection, and Planning extraction on one authoritative implementation, then pass public/historical compatibility, Windows, and the enforced same-environment performance gate before R3.
...
Next gate: Keep the R1B transfer boundary frozen while R2 extracts Lifecycle read paths; do not move Engine workflows again before the R3 checkpoint.
