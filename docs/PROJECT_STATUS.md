...
Current gate: Extend the versioned execution boundary from an existing target's non-compilation payload to target creation, Persona Compilation and profile binding, then prove a failed public import leaves no partial new target before moving guard or conflict ownership.
...
| ERIIEngine facade | `core` | `active-alpha` | `golden` | `orchestrates-current-formats` | python-matrix, windows-smoke, contract-snapshots |
| Storage and data lifecycle | `core` | `maintenance` | `golden` | `owns-current-formats` | python-matrix, windows-smoke, contract-snapshots, longitudinal |
...
Primary host facade plus private MemoryPack analysis, export assembly, snapshot-bound transfer planning, and a preflighted executor whose existing-target non-compilation payload is exception-atomic on built-in Storage adapters; SQLite post-commit errors are surfaced as indeterminate rather than reported as success, while target orchestration and Persona Compilation stay in the facade.
...
FileStorage v2 with recoverable flat-format MemoryPack payload journaling, SQLite schema 10 with single-transaction payload execution, MemoryPack 0.5.0a3, backup, restore, upgrade, import, erasure, and rebuild.
...
Next gate: Complete R2/R3 internal extraction while preserving every declared reader and atomicity invariant.
