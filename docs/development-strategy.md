...
截至当前 `0.4.0rc1.dev0` 源码快照，已完成与待完成工作按以下顺序推进；除非新证据
改变优先级：

1. **已接受：**以不可移动的完整 commit SHA 保留经验证的 `0.4.0b1` 源码检查点；
2. **已在 rc1 实现：**维护一命令 Golden Continuity Demo，作为首次采用证明；
3. **rc1 进行中：**完成公共 Interface 的审计与收窄，在不加入 v0.5 领域语义的前提下
   收口本里程碑；
4. 邀请第一批外部宿主接入，把失败转成合成回归；
5. 在文档、契约、干净安装链路与 CI 证据一致后，接受 `0.4.0rc1` 源码里程碑；
6. 完成 `0.4.0` 源码稳定里程碑；
