...
本文记录项目自 `0.4.0b1` 候选之后的发展原则。它不是固定日期、融资计划或 SLA，
...
当前源码已经完成 b1 功能，可以在不创建 GitHub Release 或分发包的情况下直接进入
rc1。进入下一阶段前只要求：

- 合并并复核 b1 文档、契约与版本身份；
- 通过 Linux/Windows 已声明矩阵、Ruff、测试、`compileall` 和纵向回放；
- 验证从固定 commit 的源码安装，并在干净环境运行 CLI；
- 可以本地构建 wheel/sdist 做工程 smoke，但不上传发行资产。

### `0.4.0rc1`：源码与首次采用收口

`rc1` 保留现有版本命名，但不表示即将上传 Release Candidate 包。

RC 不增加新关系维度、记忆类型、人格变化渠道或 v0.5 后果模型。它集中完成：

- 修复正确性、恢复性、兼容性、性能与构建缺陷；
- 明确 Python Interface 的 Golden Path、Advanced、Experimental 与 Internal 等级；
- 提供计划中的一命令 Golden Continuity Demo，并由 CI 真正运行；
...
除非新证据改变优先级，内核演进轨依次进行：

1. 冻结并验证 `0.4.0b1` 源码检查点；
2. 增加一命令 Golden Continuity Demo；
3. 审计并收窄受支持的公共 Interface；
4. 邀请第一批外部宿主接入，把失败转成合成回归；
5. 在不加入 v0.5 领域语义的前提下完成 `0.4.0rc1`；
6. 完成 `0.4.0` 源码稳定里程碑；
7. 实现最小而完整的 v0.5 Relationship Consequence 纵切；
8. 持续推进后续源码里程碑，到 `1.0` 再建立并演练正式包发布流程。

从现在起，DeepSeek、其他模型与宿主集成实验可以在 Labs 与集成轨并行进行。它们永远
不阻塞上述 1–8，也不会因为一次结果不错就自动获得持久兼容承诺。Character
