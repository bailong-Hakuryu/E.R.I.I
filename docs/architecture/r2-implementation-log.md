...
| `erii/_lifecycle/contracts.py` | Lifecycle Enum、dataclass、Request、Plan、Report 的权威定义 | 已迁移合同本体；Plan shape 尚待 Planning 接管 |
| `erii/_lifecycle/utils.py` | 指向 `erii.data_lifecycle` 权威 helper 的私有别名 | 等待 Inspection 整体迁移 |
| `erii/data_lifecycle.py` | 合同 re-export、Inspection、Planning 与执行编排 | 仍是主要实现 |

当前规模仅作导航信号：`data_lifecycle.py` 约 3505 行，`lifecycle_erasure.py` 约 2871 行；
...
1. 将 Plan shape 校验和文档构造迁到 Planning/Codec 的权威实现；
2. 提取零写入 Inspection；
