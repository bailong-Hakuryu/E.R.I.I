...
MemoryPack export. It does not need a model API key or network access.
...
4. **Portability:** User A's relationship is exported as an independently
   parseable `user-a.erii` MemoryPack.
...
| `demo-report.json` | Stable IDs, exact B baseline evidence, Persona-scope evidence, provenance references, and all four checks |
...
and its source phrase. Only User A's Pack is exported.

`demo-report.json` is an inspectable Demo artifact, not a frozen application
API. Use its `schema_version` when experimenting, but do not treat the rc1
development shape as a long-term host contract.
