...
| `0.5.0a1` | 活跃 alpha 源码里程碑 | 关系后果最小纵切已实现；继续验证迁移、采用与行为收益 |
...
当前活跃开发源码身份是 `0.5.0a1`。它已交付一条可观察、可测试的完整路径；这是
alpha 源码里程碑，不是稳定发布、生产质量或 SLA 声明：
...
当前格式身份是 FileStorage v2、SQLite schema v10 与 MemoryPack `0.5.0a1`；新 reader
可以读取 `0.4.0a8` Pack，旧 reader 会严格拒绝带新根字段的 `0.5.0a1` Pack。
