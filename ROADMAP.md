...
| `0.4.0b1` | 当前源码里程碑，功能完成 | 数据迁移、备份恢复、删除重建、长期评测与契约冻结 |
| `0.4.0rc1` | 下一源码阶段 | 缺陷、兼容、采用路径、构建和源码收口；不新增领域模型 |
| `0.4.0` / `0.4.x` | 计划源码里程碑 | 稳定角色连续性与长期记忆内核 |
...
- Python 下限提高到 3.11，包元数据支持 3.11–3.14；当前 Linux CI 覆盖 3.11 与
  3.14，Windows 运行受针对性 smoke/regression 验证。RC 必须审计并公布准确支持矩阵；
...
`rc1` 保留现有版本命名，但这里只表示源码收口检查点，不表示即将分发 Release
Candidate 包。
...
- 提供计划中的 `erii demo` Golden Continuity Demo，并把 README 入口收敛为
  “安装 → 运行 → 看见关系隔离 → 再读完整手册”；
...
- `0.4.0rc1` 不改变持久格式；只允许在内核外进行可整体删除、无心理持久化的
  Shadow 对照实验；
